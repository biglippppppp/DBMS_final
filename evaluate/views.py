...
def evaluate_detail(request, user_id, pre_user_id):
    ...
    return render(request, 'evaluate/evaluate_detail.html',
                  {'user_id': user_id, 'avg_score': avg_score, 'evaluates': evaluates, "pre_user_id": pre_user_id})
...
def write_review(request, user_id, target_id):
    ...
    if request.method == 'POST':
        ...
        api_response = api_response.json()
        return redirect('evaluate:evaluate_detail', user_id=target_id, pre_user_id=user_id)

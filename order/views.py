...
def want_order(request, user_id):
    if request.method == 'GET':
        ...
        total_pages = 50000
        page = int(page)
        range_ = range(max(1, page - 5), min(page + 6, total_pages + 1))
        return render(request, 'order/want_order.html', {'orders': orders, 'user_id': user_id, 'page_range': page_range, 'range_': range_})
...
def sale_order(request, user_id):
    if request.method == 'GET':
        ...
        return render(request, 'order/sale_order.html', {'orders': orders, 'user_id': user_id, 'page_range': page_range, 'range_': range_})
    return HttpResponseNotAllowed(['GET'])
